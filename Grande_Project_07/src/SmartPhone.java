...
public class SmartPhone {
	...
	public String duplicateCheckPhoneNum(String inputData) {
		...
		for(int i=0; i<contactlist.size(); i++) {
			while(contactlist.get(i).getPhoneNumber().equals(inputData)) {
	...
	public void addContact(Contact contact) {
		if(contact instanceof CompanyContact) {
			contactlist.add((CompanyContact)contact);
		}
		else if(contact instanceof CustomerContact) {
			contactlist.add((CustomerContact)contact);
		}
		else {
			contactlist.add(contact);
		}
		System.out.println(">>> 연락처가 저장되었습니다.("+contactlist.size()+")\n");
	}
